...
function setupCoreListeners() {
    document.getElementById("searchInput").addEventListener("input", renderDirectoryExplorer);
...
function renderDirectoryExplorer() {
    ...
    const filteredRecords = localDatabase.filter(item => {
        const matchesSearch = item.service_name.toLowerCase().includes(searchKeyword) || 
                              item.related_services.some(s => s.toLowerCase().includes(searchKeyword));
    ...
    filteredRecords.forEach(item => {
        ...
        card.innerHTML = `
            <div>
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        <h2 class="text-lg font-bold text-slate-900 tracking-tight mt-0.5">${item.service_name}</h2>
                    </div>
                    <span class="px-2.5 py-1 rounded-md text-xs font-bold ${badgeColor}">${item.price_level}</span>
                </div>
                <p class="text-xs text-slate-400 font-semibold mb-2">SEO Blueprint: <span class="italic text-slate-600">"${item.local_seo.example_title}"</span></p>
            ...
            <div class="mt-5 pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button onclick="openLeadModal('${item.id}')" class="bg-slate-900 hover:bg-indigo-600 text-white text-xs font-bold py-2 px-4 rounded-lg transition-all tracking-wide uppercase">
                    ${item.call_to_action}
                </button>
            </div>
        `;
...
function openLeadModal(nicheId) {
    const record = localDatabase.find(r => r.id === nicheId);
    if (!record) return;

    document.getElementById("formNicheId").value = record.id;
    document.getElementById("formNicheName").value = record.service_name;
    document.getElementById("modalNicheName").textContent = record.service_name;
